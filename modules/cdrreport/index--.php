...
function _moduleContent(&$smarty, $module_name)
{
    ...
    if(is_array($arrResult['cdrs']) && $total>0) {
        foreach($arrResult['cdrs'] as $key => $value) {
            ...
            $arrTmp[10] = $value[12]; // userfield
           if ($value[12] != null) {
                $esc_recfile = htmlentities($value[12], ENT_COMPAT, 'UTF-8');
                $recinfo = $oCDR ->resolveRecordingPath($value[12]);
                ...
                $recordingLink = ' BOBY ESTOU VAZIO';
            }
            $arrTmp[10] = $recordingLink;
